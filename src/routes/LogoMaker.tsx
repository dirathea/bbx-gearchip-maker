 ...
 * All paths extracted from the Illustrator template (single unit, centered at 258,258).
 * Colors are parameterized via inline styles to override the CSS classes.
 */
function BeybladeLogo({ config }: { config: LogoConfig }) {
  const { discColor, ringAccentColor, sweepColor, textColor, bgColor, spinDirection, centerImage } = config;
  ...
  return (
    ...
    >
      ...
      {/* Background */}
      <rect width="516" height="516" rx="258" fill={bgColor} />

      <g transform={flip ? "translate(516, 0) scale(-1, 1)" : undefined}>
        {/* Inner disc */}
        <circle cx="258" cy="258" r="186" fill={discColor} />

        {/* Center image overlay */}
        {centerImage && (
          <circle cx="258" cy="258" r="186" fill="url(#centerImg)" clipPath="url(#discClip)" />
        )}
        ...
        {/* Arc detail at bottom (separates disc from ring) */}
        <path
        ...
        {/* Speed/motion lines (dashed) on right side */}
        <g fill="none" stroke={sweepColor} strokeWidth="33" strokeMiterlimit="10">
          <path d="M335.95,50.22c.62.23,1.25.47,1.87.71" />
          <path
            d="M345.23,53.94c7.38,3.16,14.56,6.71,21.51,10.62"
            strokeDasharray="4 6 10 8"
          />
          <path
            d="M370.21,66.56c44.9,26.37,79.7,68.09,97.19,117.95"
            strokeDasharray="4 8 4 6 10 8"
          />
          <path d="M468.69,188.31c.21.63.42,1.27.62,1.9" />
        </g>

        {/* Three triangle markers (speed arrows) */}
...
export function LogoMaker() {
  ...
  const handleImageUpload = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      update("centerImage", reader.result as string);
    };
      ...
      <div className="flex-1 px-5 pb-6 space-y-5">
        ...
        <div>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <ImagePlus size={18} />
              {config.centerImage ? "Change Image" : "Upload Image"}
            </button>
            {config.centerImage && (
              <button
        ...
        <div>
          ...
          {activeColorTab === "presets" ? (
              ...
              {PRESETS.map((preset) => (
                <button
                  ...
                  onClick={() =>
                    setConfig((prev) => ({
                      ...prev,
                      discColor: preset.disc,
                      ringAccentColor: preset.accent,
                      sweepColor: preset.sweep,
                      textColor: preset.text,
                    }))
                  }
                  className={cn(
                    "flex flex-col items-center gap-1 rounded-lg p-2 border transition-all",
                    config.discColor === preset.disc
                ...
                >
                  <div
                    ...
                    style={{ backgroundColor: preset.disc }}
                  />
            ...
            <div className="space-y-3 rounded-xl bg-neutral-900/50 p-4 border border-neutral-800">
              <ColorInput
                label="Disc"
                value={config.discColor}
                onChange={(v) => update("discColor", v)}
              />
              <ColorInput
                label="Accent Ring"
                value={config.ringAccentColor}
                onChange={(v) => update("ringAccentColor", v)}
              />
              <ColorInput
                label="Sweep Lines"
                value={config.sweepColor}
                onChange={(v) => update("sweepColor", v)}
              />
              <ColorInput
                label="Text"
